...
namespace TestyDroid
{
    ...
    public class MicrosoftAndroidEmulator : IAndroidEmulator
    {
        ...
        public MicrosoftAndroidEmulator(ILogger logger, string emulatorExePath, string avdName, IAndroidDebugBridgeFactory adbFactory, Guid id, int? consolePort, SingleInstanceMode instanceMode = SingleInstanceMode.KillExisting, bool noBootAnim = true, bool noWindow = true)
        {
            _logger = logger;
            _avdName = avdName;
        ...
        private void StartInstance()
        {
            ...
            _instanceInfo = _instanceResolver.EnsureInstance(_instanceMode, _consolePort, (argsBuilder) => {
                argsBuilder.AppendFormat("/sku:{0}", "Android");

                argsBuilder.AppendFormat(" launch /id:{0}", _avdName);
